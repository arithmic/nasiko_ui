    ...
    return MergeSemantics(
      child: Semantics(
        ...
        child: MouseRegion(
          ...
          // onHover (real pointer motion) rather than onEnter: after every
          // scroll frame the mouse tracker re-hit-tests and fires onEnter on
          // whichever row slid under the stationary cursor, which made the
          // highlight hop row-to-row (flicker) while scrolling. Hover events
          // are only generated by actual mouse movement, so scrolling leaves
          // the active row alone — the cmdk/VS Code list behavior.
          onHover: (_) => onHover(),
          child: GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              curve: motion.enter,
              decoration: BoxDecoration(
                // Zero-alpha hover color, NOT Colors.transparent: lerping from
                // transparent black drags the tween through semi-transparent
                // dark grays, flashing the row darker on every hover change.
                // Keeping the hue fixed animates alpha only — a clean fade.
                color: isActive
                    ? colors.backgroundSurfaceHover
                    : colors.backgroundSurfaceHover.withValues(alpha: 0),
